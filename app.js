    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="card-bg p-8 border border-white/5 shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">Total Revenue</h3>
    ...
    return (
        <div className="min-h-screen pb-32">
            <header className="p-6">
                <h1 className="text-2xl font-black gold-text italic uppercase leading-none">INKFLOW PRO</h1>
            ...
            <main className="p-4">
                            ...
                            {appointments.filter(a => a.date === todayStr).length === 0 ? <p className="text-center opacity-20 py-10 italic">No jobs today</p> :
                                appointments.filter(a => a.date === todayStr).map(app => (
                                    <div key={app.id} className="bg-[#0f172a] p-4 rounded-2xl mb-3 flex justify-between items-center border-l-4 border-yellow-500">
                                        <div><p className="font-bold text-sm text-white">{app.client}</p><p className="text-[10px] text-slate-500 uppercase font-bold">{app.time} | {app.style}</p></div>
                ...
                {activeTab === 'biz' && (
                    <BusinessOverview appointments={appointments} currentMonthName={currentMonthName} currentYear={currentYear} />
                )}
                ...
                {activeTab === 'crm' && (
                    <div className="space-y-3">
                        {clients.length === 0 ? <p className="text-center opacity-20 py-20 italic">No clients yet</p> :
                            clients.map(c => (
                                <div key={c.id} onClick={() => setSelectedClientData(c)} className="card-bg p-5 flex justify-between items-center border border-slate-800/50 active:scale-[0.98] transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 gold-bg rounded-full flex items-center justify-center text-black font-black shadow-lg">{c.name[0]}</div>
                                        <div>
                                            <p className="font-bold text-white">{c.name}</p>
                                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{c.social || '@no_tag'}</p>
                                        </div>
                                    </div>
            ...
            {selectedClientData && (
                <div className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-xl flex items-center p-6 animate-in fade-in zoom-in duration-300" onClick={() => setSelectedClientData(null)}>
                    <div className="card-bg w-full p-8 rounded-[40px] border border-slate-800 shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-3 bg-[#0a0f1d] p-6 rounded-3xl border border-slate-800 shadow-inner mb-6">
                            <div className="flex justify-between items-center"><span className="text-slate-600 text-[9px] font-black uppercase">Phone</span><span className="text-white font-bold text-sm tracking-widest">{selectedClientData.phone || '—'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-slate-600 text-[9px] font-black uppercase">Email</span><span className="text-white font-bold text-xs">{selectedClientData.email || '—'}</span></div>
                        </div>
                        
                        <button onClick={() => setSelectedClientData(null)} className="w-full bg-slate-800 text-slate-500 font-black p-5 rounded-2xl uppercase tracking-[0.2em] text-[10px] active:scale-95 transition-all">Close Profile</button>
                    </div>
                </div>
            )}

            {/* MODAL ZA ZAKAZIVANJE - LAYOUT FIX */}
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-end animate-in slide-in-from-bottom duration-500" onClick={() => setIsModalOpen(false)}>
                    <div className="card-bg w-full p-8 rounded-t-[40px] border-t border-slate-800 max-h-[95vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-[#0a0f1d] border border-slate-800 p-3 rounded-2xl text-center shadow-inner relative h-20 flex flex-col justify-center">
                                    <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest absolute top-2 left-0 right-0">Time</p>
                                    <input type="time" className="w-full bg-transparent font-black text-yellow-500 text-center outline-none text-lg mt-2" 
                                        onChange={e => setNewEntry({...newEntry, time: e.target.value})} />
                                </div>
                                <div className="bg-[#0a0f1d] border border-slate-800 p-3 rounded-2xl text-center shadow-inner relative h-20 flex flex-col justify-center">
                                    <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest absolute top-2 left-0 right-0">Price</p>
                                    <div className="flex items-center justify-center gap-1 mt-2">
                                        <input type="number" placeholder="0" className="w-12 bg-transparent font-black text-white text-center outline-none text-lg" 
