...
function App() {
    ...
    // Dinamički ključevi za svakog usera
    ...
    // Učitavanje specifičnih podataka pri logovanju
    useEffect(() => {
        if (user) {
            setAppointments(JSON.parse(localStorage.getItem(storageKeyApps) || '[]'));
            setClients(JSON.parse(localStorage.getItem(storageKeyClients) || '[]'));
            localStorage.setItem('inkflow_logged_user', JSON.stringify(user));
        } else {
            setAppointments([]);
            setClients([]);
        }
    }, [user, storageKeyApps, storageKeyClients]);

    // Čuvanje specifičnih podataka
    useEffect(() => {
        if (user) {
    ...
    }, [appointments, clients, user, storageKeyApps, storageKeyClients]);
    ...
    return (
        <div className="min-h-screen pb-32">
            <header className="p-6 flex justify-between items-center">
                ...
                <button onClick={() => {localStorage.removeItem('inkflow_logged_user'); setUser(null);}} className="bg-slate-800 text-slate-400 px-3 py-1 rounded-full text-[8px] uppercase font-black">Out</button>
            </header>
            ...
            <main className="p-4">
                ...
                {activeTab === 'crm' && (
                    <div className="space-y-3">
                        ...
                        {clients.map(c => (
                            <div key={c.id} className="card-bg p-5 flex justify-between items-center border border-slate-800" onClick={() => setSelectedClientData(c)}>
                ...
                <div className="fixed inset-0 z-[140] bg-black/95 flex items-center p-6">
                    <div className="card-bg w-full p-8 rounded-[40px] border border-slate-800">
                        ...
                        <div className="space-y-4">
                            ...
                            <button onClick={() => setIsEditClientOpen(false)} className="w-full text-slate-600 font-black p-2 uppercase text-[9px] mt-2 text-center">Cancel</button>
                        </div>
                    </div>
                </div>
            )}

            {/* ADD SESSION MODAL - SA SVIM POLJIMA */}
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] bg-black/95 flex items-center p-6" onClick={() => setIsModalOpen(false)}>
                    <div className="card-bg w-full p-8 rounded-[40px] border border-slate-800" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            <input type="text" placeholder="Client Name" className="w-full bg-[#0a0f1d] border border-slate-800 p-5 rounded-2xl text-white text-center" onChange={e => setNewEntry({...newEntry, client: e.target.value})} />
                            <input type="text" placeholder="Tattoo Style" className="w-full bg-[#0a0f1d] border border-slate-800 p-5 rounded-2xl text-white text-center" onChange={e => setNewEntry({...newEntry, style: e.target.value})} />
                            <div className="grid grid-cols-2 gap-3">
                                <input type="time" className="bg-[#0a0f1d] border border-slate-800 p-5 rounded-2xl text-white text-center" onChange={e => setNewEntry({...newEntry, time: e.target.value})} />
                                <input type="number" placeholder="Price €" className="bg-[#0a0f1d] border border-slate-800 p-5 rounded-2xl text-white text-center" onChange={e => setNewEntry({...newEntry, price: e.target.value + '€'})} />
                            </div>
                            <button onClick={handleSaveAppointment} className="w-full gold-bg text-black font-black p-5 rounded-2xl uppercase mt-4 shadow-2xl">Confirm</button>
                        </div>
                    </div>
                </div>
            )}
