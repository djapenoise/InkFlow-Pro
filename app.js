...
// --- BUSINESS TAB ANALIZA (Tvoj originalni kod) ---
...
// --- GLAVNA APLIKACIJA (Zadržan tvoj stil i logika uz popravku privatnosti podataka) ---
function App() {
    ...
    // DINAMIČKI KLJUČEVI: Svaki user dobija svoj prostor u memoriji
    ...
    // Inicijalizacija stanja isključivo iz ključa ulogovanog korisnika
    const [appointments, setAppointments] = useState([]);
    const [clients, setClients] = useState([]);

    // Kada se korisnik promeni (Login/Logout), učitaj njegove podatke
    useEffect(() => {
        if (user) {
        ...
        <div className="min-h-screen pb-32">
            <header className="p-6 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    <button onClick={() => {localStorage.removeItem('inkflow_logged_user'); setUser(null);}} className="bg-slate-800 text-slate-400 p-2 rounded-full text-[8px] uppercase font-black shadow-lg">Out</button>
                    {activeTab === 'crm' && (
            ...
            {/* MODALI (Tvoj originalni kod i dizajn) */}
